...
export default function SalesMonthlyChart({ filters }) {
  const { data, loading, error } = useApi(() => getVentasMensuales(filters), [JSON.stringify(filters)])
  const rows = data || []
  ...
  return (
    <div className="bg-white p-4 rounded shadow">
      <h3 className="mb-2">Ventas mensuales</h3>
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={rows}>
          <CartesianGrid stroke="var(--grid, rgba(148, 163, 184, 0.16))" vertical={false} />
          <XAxis dataKey="nombre_mes" tick={{ fill: 'var(--muted-axis, #92a5c7)' }} axisLine={false} tickLine={false} />
          ...
          <Bar dataKey="total_ventas" fill="#22d3ee" radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
